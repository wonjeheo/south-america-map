import { clearCities, loadCities, setupCityEvents } from "./city.js";
...
import { getAccessMode, isAdminMode, setAccessMode } from "./state.js";
...
function setLoginButtonBusy(isBusy) {
  ...
  btnLoginGoogle.textContent = isBusy
    ? "로그인 중..."
    : "Google 로그인 (관리자)";
}
...
function applyModeUI(mode) {
  ...
  if (mode === "admin") {
    ...
    privacyNotice.textContent =
      "관리자 모드: 정확한 일정·숙소·비용을 포함한 비공개 원본을 보고 있습니다.";
...
async function loadMode(mode) {
  ...
  loadingPromise = (async () => {
    ...
    try {
      ...
      if (
        mode === "guest" &&
        Object.keys(cityMarkers).length === 0
      ) {
        document.getElementById("timeline-empty").textContent =
          "아직 공개된 여행 데이터가 없습니다.";
      }
    } catch (error) {
      console.error("[Data] Failed to load travel data", error);
...
btnPublish.addEventListener("click", async () => {
  ...
  try {
    ...
    publishStatus.textContent =
      `공개 지도 갱신 완료: ` +
      `도시 ${result.cityCount}개, ` +
      `경로 ${result.routeCount}개`;
  } catch (error) {
    console.error(
      "[Publish] Public snapshot update failed",
      error
    );
...
onAuthStateChanged(auth, async user => {
  if (user) {
    ...
    console.warn(
      "[Auth] Non-admin account attempted administrator sign-in"
    );
...
document
  .getElementById("btn-southamerica")
  .addEventListener("click", () => {
    map.flyTo(
      [-10, -65],
      4.3,
...
function startClock() {
  const koreaElement =
    document.getElementById("time-korea");
  ...
  const tick = () => {
    ...
    const koreaTime = now.toLocaleTimeString(
      "ko-KR",
      {
        hour: "2-digit",
        minute: "2-digit",
    ...
    if (
      !targetCity ||
      !Array.isArray(targetCity.Coords)
    ) {
    ...
    let offsetHours = Math.round(
      Number(targetCity.Coords[1]) / 15
    );

    if (
      ["인천", "서울"].includes(targetCity.City)
    ) {
      offsetHours = 9;
    }
    ...
    localElement.textContent =
      `📍 ${targetCity.City} ${formatTime(localDate)}`;
  };
